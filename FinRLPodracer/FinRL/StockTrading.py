...
class StockEnvDOW5:
    def __init__(self, cwd='./data/StockEnvDOW5', gamma=0.997,
                 turbulence_thresh=int(1e4), min_stock_rate=0.1,
                 max_stock=1e2, initial_capital=1e6, buy_cost_pct=1e-3, sell_cost_pct=1e-3,
                 reward_scaling=2 ** -11,
                 initial_stocks=None, if_eval=False):
        self.min_stock_rate = min_stock_rate
        self.if_eval = if_eval

        '''load data'''
        # data_arrays = self.load_data(cwd)
        # cwd = './data/StockEnvDOW5'
        beg_i, mid_i, end_i = 0, 7 * (2 ** 12), 46018
        slice_i0, slice_i1 = (mid_i, end_i) if if_eval else (beg_i, mid_i)

        self.price_ary = np.load(f'{cwd}/price.npy')  # self.price_ary.std()=1236
        self.tech_ary = np.load(f'{cwd}/tech.npy')  # self.tech_ary.std()=1121
        self.turbulence_ary = np.load(f'{cwd}/turb.npy')  # self.turbulence_ary.std()=199
        self.price_ary = self.price_ary[slice_i0:slice_i1]
        self.tech_ary = self.tech_ary[slice_i0:slice_i1]
        self.turbulence_ary = self.turbulence_ary[slice_i0:slice_i1]
        '''load data, end'''

        import matplotlib.pyplot as plt
        plt.plot(self.turbulence_ary)
        plt.plot(self.sigmoid_sign(self.turbulence_ary, turbulence_thresh))
        plt.show()

        self.turbulence_ary = self.turbulence_ary
        self.turbulence_bool = (self.turbulence_ary > turbulence_thresh).astype(np.float32)

        """norm"""
        # self.price_ary = self.price_ary * 2 ** -9
        self.tech_ary = self.tech_ary * 2 ** -9
        self.turbulence_ary = self.sigmoid_sign(self.turbulence_ary, turbulence_thresh) * 2 ** -10
        ...
        self.env_name = 'StockEnvDOW30-v1'
        self.state_dim = 1 + 2 + 2 * stock_dim + self.tech_ary.shape[1]
        # amount 1 + (turbulence_ary, turbulence_bool) 2 + (price, stock) 2 * stock_dim + tech
        self.action_dim = stock_dim
        self.max_step = self.price_ary.shape[0] - 1
        self.if_discrete = False
        self.target_return = 1.1
    ...
    def get_state(self, price):
        state = np.hstack((max(self.amount, 1e4) * (2 ** -17),
                           price * (2 ** -9),
...
class StockEnvDOW30(gym.Env, ABC):
    metadata = {'render.modes': ['human']}  # gym.Env

    def __init__(self, cwd='./envs/FinRL', gamma=0.99, turbulence_thresh=380,
                 max_stock=1e2, initial_capital=1e6, buy_cost_pct=1e-3, sell_cost_pct=1e-3,
                 start_date='2009-01-01', end_date='2019-01-01', env_eval_date='2021-01-01',
                 ticker_list=None, tech_indicator_list=None, initial_stocks=None, reward_scaling=2 ** -13,
                 if_eval=False):
        super(StockEnvDOW30, self).__init__()
        # cwd = '/home/yonv/code/ElegantRL/envs/FinRL'
        # cwd = '/raid/yanglet/yonv/ElegantRL/envs/FinRL'

        self.price_ary, self.tech_ary, self.turbulence_ary, self.trade_data_list = self.load_data(
            cwd, if_eval, ticker_list, tech_indicator_list, start_date, end_date, env_eval_date, )
        ...
        # environment information
        self.env_name = 'StockEnvDOW30-v2'
        self.state_dim = 1 + 2 * stock_dim + self.tech_ary.shape[1] + 1
        self.action_dim = stock_dim
        self.max_step = len(self.price_ary) - 1
        self.if_discrete = False
        self.target_return = 1.5
        self.episode_return = 0.0
        self.reward_scaling = reward_scaling

        # gym.Env
        self.observation_space = gym.spaces.Box(low=-2000, high=2000, shape=(self.state_dim,), dtype=np.float32)
    ...
    def reset(self):
        ...
        self.stocks = self.initial_stocks + rd.randint(0, 64, size=self.initial_stocks.shape)
        self.amount = self.initial_capital * rd.uniform(0.9, 1.1) - (self.stocks * price).sum()
    ...
    def step(self, actions):
        ...
        if self.turbulence_ary[self.day] > self.turbulence_thresh:  # sell all when turbulence
            self.amount += (self.stocks * price).sum() * (1 - self.sell_cost_pct)
            self.stocks[:] = 0
        else:
    ...
    def get_state(self, price):
        return np.hstack((min(self.amount, 5e4) * 2 ** -9,
                          price,
                          self.stocks,
                          self.tech_ary[self.day],
                          self.turbulence_ary[self.day] * 2 ** -3,
    ...
    def load_data(self, cwd='./envs/FinRL', if_eval=None,
        ...
        data_name = 'DOW30'  # 'NAS89
        raw_data_path = f'{cwd}/StockEnv_{data_name}_raw_data.df'
        processed_data_path = f'{cwd}/StockEnv_{data_name}_processed_data.df'
        data_path_array = f'{cwd}/StockEnv_{data_name}_arrays_float16.npz'
        # trade_data_path = f'{cwd}/StockEnv_{data_name}_trading_data.df'  # check sth wrong with trading_data
        ...
        if os.path.exists(data_path_array):
            ...
            trade_data_ary = None  # load_dict['trade_data_ary']  # type=str
            # print(np.where(trade_data_ary > '2016-01-01')[0][0])
            # print(np.where(np.unique(trade_data_ary) > '2016-01-01')[0][0])
        else:
            ...
            # plan to cancel data_split, change to train_data, eval_data = data[:inx], data[ind:]
            train_df = data_split(processed_df, start_date, end_date)
            eval_df = data_split(processed_df, end_date, env_eval_date)

            (train_price_ary, train_tech_ary, train_turbulence_ary
             ) = self.convert_df_to_arys(train_df, tech_indicator_list)
            (eval_price_ary, eval_tech_ary, eval_turbulence_ary
             ) = self.convert_df_to_arys(eval_df, tech_indicator_list)

            trade_data_ary = np.array(processed_df.date.unique())  # sth wrong with trade_data

            np.savez_compressed(file=data_path_array,
                                ...
                                trade_data_ary=trade_data_ary,  # type=str
                                )
        ...
        return price_ary, tech_ary, turbulence_ary, trade_data_ary
    ...
    @staticmethod
    def load_data_date_list(path, start, end, ticker_list):
    ...
    @staticmethod
    def convert_df_to_arys(df, tech_indicator_list):
        price_ary = list()
        tech_ary = list()
        turbulence_ary = list()
        ...
        turbulence_ary = np.array(turbulence_ary).mean(axis=1)
        print(f'| price_ary.shape: {price_ary.shape}'
              f'| tech_ary.shape: {tech_ary.shape}'
              f'| turbulence_ary.shape: {turbulence_ary.shape}')
        return price_ary, tech_ary, turbulence_ary

    def trade_prediction(self, args, _torch):
        ...
        df_account_value = pd.DataFrame({'date': self.trade_data_list, 'account_value': episode_returns})
    ...
    def backtest_plot(self, account_value, baseline_start, baseline_end, baseline_ticker="^DJI"):
        import pyfolio
        df = account_value.copy(deep=True)
...
class StockEnvNAS74(gym.Env, ABC):
    metadata = {'render.modes': ['human']}  # gym.Env

    def __init__(self, cwd='./envs/FinRL',
                 gamma=0.997, turbulence_thresh=700, min_stock_rate=0.0,
                 max_stock=1e2, initial_capital=1e6, buy_cost_pct=1e-3, sell_cost_pct=1e-3,
                 start_date='2008-03-19', end_date='2016-01-01', data_gap=1, reward_scaling=2 ** -15,
                 ticker_list=None, tech_indicator_list=None, initial_stocks=None, if_eval=False):
        cwd = '.'  # '/mnt/sdb1/Yonv/code/ElegantRL/envs/FinRL'

        self.min_stock_rate = min_stock_rate
        beg_i, mid_i, end_i = 0, int(1446), int(1962)  # 74%
        ...
        # if_eval = True
        # self.price_ary: mean, std, min, max = 72, 133, 0.97, 1467
        # self.tech_ary:  mean, std, min, max = 51, 102, -38, 1524
        # turbulence_ary: mean, std, min, max = 119, 105, 18, 898
        self.tech_ary = self.tech_ary * 2 ** -7
        self.turbulence_bool = (turbulence_ary > turbulence_thresh).astype(np.float32)
        self.turbulence_ary = (self.sigmoid_sign(turbulence_ary, turbulence_thresh) * 2 ** -9).astype(np.float32)
    ...
    def load_data(self, cwd='./envs/FinRL', ticker_list=None, tech_indicator_list=None,
        ...
        if os.path.exists(data_path_array):
            ...
            raw_data2_df = self.get_raw_data2_df(raw_data1_path, raw_data2_path,
                                                 ticker_list, tech_indicator_list)
            ...
            raw_data2_df = data_split(raw_data2_df, '2008-03-19', '2016-01-01')
            # print(raw_data2_df)

            # print(raw_data2_df.date)  # to show the start_date and end_date
            price_ary, tech_ary, turbulence_ary = self.convert_df_to_ary(raw_data2_df, tech_indicator_list)
    ...
    def get_raw_data2_df(self, raw_data1_path, raw_data2_path,
                         ticker_list, tech_indicator_list):
        if os.path.exists(raw_data2_path):
            raw_data2_df = pd.read_pickle(raw_data2_path)  # DataFrame of Pandas
            print(f'| raw_data2_df.columns.values: {raw_data2_df.columns.values}')
            ...
            raw_data2_df = fe.preprocess_data(raw_df)
            raw_data2_df.to_pickle(raw_data2_path)
            print("| FeatureEngineer: finish processing data")
        ...
        # processed_data_path = f'{cwd}/dow_30_daily_2000_2021.csv'
        # if os.path.exists(processed_data_path):
        #     raw_data2_df = pd.read_csv(processed_data_path)
        return raw_data2_df
    ...
    def deal_with_split_or_merge_shares(self, price_ary, tech_ary):
        ...
        tech_ary_shape = tech_ary.shape  # NAS74: (1962, 74, 7)
        for tech_i1 in range(tech_ary_shape[1]):  # fill_nan_with_next_value
            for tech_i2 in range(tech_ary_shape[2]):
                tech_item = tech_ary[:, tech_i1, tech_i2]
                tech_item = self.fill_nan_with_next_value(tech_item)
                tech_ary[:, tech_i1, tech_i2] = tech_item

        tech_ary = tech_ary.reshape((tech_ary_shape[0], -1))
    ...
    def convert_df_to_ary(df, tech_indicator_list):
        ...
        price_ary = np.array(price_ary)
        turbulence_ary = np.array(turbulence_ary).mean(axis=1)
        print(f'| ary.shape price, tech, turbulence: {price_ary.shape} {tech_ary.shape} {turbulence_ary.shape}')
        return price_ary, tech_ary, turbulence_ary
